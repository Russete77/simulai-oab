      ...
      <div className="min-h-screen bg-bg">
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <nav className="flex items-center gap-2 text-sm text-ink-2 mb-8">
            ...
            <span>/</span>
            <span className="text-ink-1">Simulado OAB Online</span>
          </nav>
          ...
          <div className="text-center mb-16">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              ...
              <Link
                href="/pricing"
                className="inline-flex items-center gap-2 px-8 py-4 bg-surface-2 text-ink-1 font-semibold rounded-xl hover:bg-surface-2 transition-all text-lg"
              >
                Ver Planos
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-16">
            {[
              ...
              <div key={i} className="bg-surface border rounded-xl p-4 text-center">
                <stat.icon className="w-6 h-6 text-accent mx-auto mb-2" />
                <div className="text-2xl font-bold text-ink-1">{stat.value}</div>
          ...
          <section className="mb-16">
            <h2 className="text-3xl font-bold text-ink-1 mb-8 text-center">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {[
                ...
                <div key={i} className="bg-surface border rounded-xl p-6 hover:border-accent/50 transition-all">
                  <mode.icon className="w-8 h-8 text-accent mb-4" />
                  <h3 className="text-lg font-semibold text-ink-1 mb-2">{mode.title}</h3>
                  <p className="text-ink-2 text-sm">{mode.desc}</p>
                </div>
              ))}
            </div>
          </section>
          ...
          <section className="mb-16">
            <h2 className="text-3xl font-bold text-ink-1 mb-6">
              Por Que Fazer Simulados OAB é Essencial para Passar na Prova
            </h2>
            <div className="bg-surface border rounded-xl p-8">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
              {subjects.map((subject) => (
                <Link
                  ...
                  className="flex items-center gap-3 bg-surface border rounded-xl p-4 hover:border-accent/50 hover:bg-surface transition-all group"
                >
                  <span className="text-2xl">{subject.icon}</span>
                  <div>
                    <div className="text-ink-1 font-medium group-hover:text-accent transition-colors">{subject.name}</div>
                    <div className="text-sm text-ink-2">Ver questões →</div>
                  </div>
                </Link>
              ))}
            </div>
          ...
          <section className="mb-16">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <div>
                <h3 className="text-xl font-semibold text-ink-1 mb-4">Gabaritos Comentados</h3>
                <div className="space-y-2">
                  {examIds.slice(0, 10).map((examId) => (
                    <Link
                      key={examId}
                      href={`/gabarito/${examId}`}
                      className="block text-accent hover:text-accent transition-colors"
                    >
                      Gabarito {examId.toUpperCase()} →
                    </Link>
                  ))}
                  <Link href="/gabarito" className="block text-ink-2 hover:text-ink-1 transition-colors mt-2">
                    Ver todos os gabaritos →
                  </Link>
                </div>
              </div>
              <div>
                <h3 className="text-xl font-semibold text-ink-1 mb-4">Simulados por Exame</h3>
                <div className="space-y-2">
                  {examIds.slice(0, 10).map((examId) => (
                    <Link
                      key={examId}
          ...
          <section className="text-center bg-gradient-to-r from-blue-600/20 to-purple-600/20 border-accent rounded-2xl p-12">
            ...
            <p className="text-ink-2 text-lg mb-8 max-w-2xl mx-auto">
            ...
            <div className="grid md:grid-cols-3 gap-8 text-sm">
              <div>
                ...
                <div className="space-y-1">
                  <Link href="/simulado-oab-online" className="block text-ink-2 hover:text-ink-1 transition-colors">Simulado OAB Online</Link>
              ...
              <div>
                <h3 className="text-ink-1 font-semibold mb-3">Matérias OAB</h3>
                <div className="space-y-1">
                  {subjects.slice(0, 6).map((s) => (
                    <Link key={s.slug} href={`/materias/${s.slug}`} className="block text-ink-2 hover:text-ink-1 transition-colors">{s.name}</Link>
                  ))}
                </div>
              </div>
              <div>
                <h3 className="text-ink-1 font-semibold mb-3">Blog OAB</h3>
                <div className="space-y-1">
                  <Link href="/blog/como-passar-na-oab-primeira-fase" className="block text-ink-2 hover:text-ink-1 transition-colors">Como passar na OAB</Link>
                  <Link href="/blog/materias-mais-cobradas-oab" className="block text-ink-2 hover:text-ink-1 transition-colors">Matérias mais cobradas</Link>
                  <Link href="/blog" className="block text-ink-2 hover:text-ink-1 transition-colors">Ver todo o blog →</Link>
                </div>
              </div>
            </div>
          </footer>
