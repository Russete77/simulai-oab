...
/**
 * Renderiza o ranking. Recebe `initialData` já buscado no servidor (SSR/ISR),
 * então o HTML é indexável pelo Google. Após hidratar, faz um fetch autenticado
 * para destacar a posição do usuário logado. Para visitantes/crawler, o fetch
 * falha silenciosamente e o ranking público permanece.
 */
export function LeaderboardClient({ initialData }: { initialData: LeaderboardData }) {
  ...
  useEffect(() => {
    ...
    async function enrich() {
      try {
        ...
        if (!response.ok) return; // visitante deslogado: mantém dados públicos
  ...
  const accuracy = data.currentUserRank && data.currentUserRank.totalQuestions > 0
    ...
    <>
      ...
      <Card variant="glass" className="overflow-hidden">
        <div className="p-6 border-b border">
          <h2 className="text-xl font-bold text-ink-1">Top {data.leaderboard.length} Estudantes</h2>
        </div>
