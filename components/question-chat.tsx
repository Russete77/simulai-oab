...
export function QuestionChat({ questionId, onClose }: QuestionChatProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      ...
      >
        {/* Header */}
        <div className="p-4 border-b border flex items-center justify-between flex-shrink-0">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {messages.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center px-4">
              ...
              <h4 className="text-ink-1 font-semibold mb-2">
                Como posso ajudar?
              </h4>
              ...
              <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-2 w-full max-w-lg">
                {[
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-surface-2 hover:bg-surface-2 border rounded-lg text-sm text-ink-1/80 hover:text-ink-1 transition-all text-left"
                  >
                    {suggestion}
                  </button>
                ))}
                ...
                >
                  ...
                  {msg.role === "user" && (
                    <div className="w-8 h-8 rounded-full bg-surface-2 flex items-center justify-center flex-shrink-0">
                      <User className="w-4 h-4 text-ink-1" />
                    </div>
                  )}
                </div>
              ))}
        ...
        <div className="p-4 border-t border flex-shrink-0">
          <div className="flex gap-2">
            <input
              ...
              disabled={loading}
              className="flex-1 bg-surface-2 border rounded-xl px-4 py-3 text-ink-1 placeholder-navy-500 focus:outline-none focus:border-accent focus:ring-1 focus:ring-accent disabled:opacity-50"
            />
            <Button
              variant="primary"
              onClick={sendMessage}
