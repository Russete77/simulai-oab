...
|---|---|---|
| Questões duplicadas | 2.240 (38,1%) | alta |
| `examPhase` com valor inválido | 2.315 (39,4%) | média |
| `questionNumber` acima de 80 | 2.262 (38,5%) | média — consequência da duplicação |
| Citação de lei não confiável | 5.856 explicações | alta — **já mitigado** |
| Gabarito aparentemente errado | ao menos 1 | a investigar |
| `successRate` nunca populado | 5.875 | baixa |

---

## 1 · Duplicação — 2.240 questões
...
- **2.229 grupos** de enunciado repetido (2.218 aparecem 2x, 11 aparecem 3x)
- **2.227 grupos estão no mesmo exame** — só 2 cruzam exames diferentes
- Sobra de **2.240 cópias excedentes**
...
### Passo 6, em destaque: 2.240 URLs vão morrer

Cada questão removida tem uma URL pública `/questoes/[id]`, presente no
sitemap e possivelmente já indexada. Apagar sem mais nada cria **2.240 novos
...
Google veja o site perdendo 2.240 páginas de uma vez.
...
Sem as duplicatas, o banco tem cerca de **3.635 questões únicas**.
...
- **477 caem em "Art. 5º da CF"**, o artigo curinga de quando não se sabe
...
**Para trazer de volta:** regerar só o campo `baseLegal` com modelo maior e
uma passada de verificação contra o texto da lei. É lote, roda uma vez.
...
Das 3.892 explicações que afirmam qual alternativa é a correta:

```
concordam com o gabarito .... 3.890
divergem ....................      2
```

E das 2 divergências, **uma é falso positivo meu** — a questão pergunta "NÃO
é correto afirmar que", e a IA acerta ao dizer "a alternativa A está
incorreta". Meu detector leu isso como discordância.

O raciocínio das explicações é sólido. O problema era estreito.
...
- **Julgar a correção jurídica** de 3.635 questões
